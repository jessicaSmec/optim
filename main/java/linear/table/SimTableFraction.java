...
public class SimTableFraction implements  SimTable {
    ...
    public SimTableFraction getSimTable() {
        SimTableFraction tab = new SimTableFraction(table,base, free);
        tab.setGoal(goal);
        return tab;
    }
        ...
        for(int i =0; i<this.table.length;i++){
            for(int j =0; j<this.table[0].length;j++){
               // arr[i][j] = this.table[i][j];
                arr[i][j] = new Fraction(this.table[i][j].getNumenator(),this.table[i][j].getDenominator());
            } }
...
// строку b rjgbhyenm
    ...
    public void minPositive1(Fraction[] b, Fraction[] arr){
        /*
        Fraction elem = arr[0];
        int index = 0;
        for(int i = 0; i < arr.length; i++){
        ...
        Fraction elem = new Fraction(arr[0].getNumenator(), arr[0].getDenominator());
        Fraction zero = new Fraction(0,0);
        if(elem.compareTo(zero)==0){
            elem=Fraction.valueOf(-0.00001);
        }
        int index = 0;
        for(int i = 0; i < arr.length; i++){

            Fraction arr_i=new Fraction(arr[i].getNumenator(), arr[i].getDenominator());
            Fraction b_i=new Fraction(b[i].getNumenator(), b[i].getDenominator());
            if(arr_i.compareTo(zero)==0){
                arr_i=Fraction.valueOf(-0.00001);
            }

            if(b_i.compareTo(zero)==0){
                b_i=Fraction.valueOf(-0.00001);;
            }

            if(arr[index].compareTo(zero)>0 ||
                    (arr[i].compareTo(zero)<0 &&
                            (b[index].divide(elem, 32, RoundingMode.HALF_UP).compareTo(b_i.divide(arr_i, 32, RoundingMode.HALF_UP))>0
                    ))
            ){

                elem = new Fraction(arr_i.getDenominator(), arr_i.getDenominator());
                index = i;
            }
        }

        // mainElem = elem;
        mainElem = new Fraction(arr[index].getNumenator(), arr[index].getDenominator());
        mainColumn = index;
